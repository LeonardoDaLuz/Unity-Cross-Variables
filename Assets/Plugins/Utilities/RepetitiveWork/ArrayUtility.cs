...
public class ArrayUtility : MonoBehaviour {
    ...
    public delegate void ArrayOperationDelegate<T>(T element);

    public static void ArrayOperation<T>(T[] array, ArrayOperationDelegate<T> _operation)
    {
        for (int i = 0; i < array.Length; i++)
        {
            _operation( array[i]);
    ...
    public static void ArrayOperation<T>(List<T> array, ArrayOperationDelegate<T> _operation)
    {
